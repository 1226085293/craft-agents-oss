...
export function resolveCustomEndpointPayload(params: {
  ...
  const { activePreset, baseUrl, customApi, supportsImages, brandedOpenAiCompatPresets, fallbackPiAuthProvider } = params
  ...
  return {
    customEndpoint: isCustomEndpoint ? { api: effectiveApi, ...(supportsImages ? { supportsImages: true } : {}) } : undefined,
