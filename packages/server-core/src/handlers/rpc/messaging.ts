...
export function registerMessagingHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.messaging.TEST_QQ, async (
    _ctx,
    creds: { appId: string; appSecret: string },
  ) => {
    return registry.testQQCredentials(creds)
  })

  server.handle(RPC_CHANNELS.messaging.SAVE_QQ, async (
    ctx,
    creds: { appId: string; appSecret: string; mainQqOpenIds?: string[] },
  ) => {
    if (!ctx.workspaceId) throw new Error('Missing workspaceId')
    await registry.saveQQCredentials(ctx.workspaceId, creds)
    return { success: true }
  })
  ...
  server.handle(RPC_CHANNELS.messaging.SAVE_WECHAT, async (
    ...
    if (!ctx.workspaceId) throw new Error('Missing workspaceId')
    await registry.saveWeChatCredentials(ctx.workspaceId, creds)
    return { success: true }
  })
  ...
  server.handle(RPC_CHANNELS.messaging.WECHAT_SUBMIT_VERIFYCODE, async (ctx, code: string) => {
    if (!ctx.workspaceId) throw new Error('Missing workspaceId')
    await registry.submitWeChatVerifyCode(ctx.workspaceId, code)
    return { success: true }
  })
